...
class NutritionInputScreen(BoxLayout):
    def save_data(self):
        data = {
            ...
            "total_fats": self.ids.total_fats_input.text,  # From Joko
            "saturated_fats": self.ids.saturated_fats_input.text,   # From Joko
            ...
            "sugar": self.ids.sugar_input.text,   # From Joko
    ...
    def on_kv_post(self, base_widget):
        input_fields = [
            ...
            self.ids.protein_input,
            self.ids.calcium_input,
            self.ids.fats_input,
...
class FoodLogScreen(BoxLayout):
    ...
    def log_food(self):
        def save_food_with_image(image_path=None):
            ...
            # Generate unique image filename and copy to app directory if image selected
            image_filename = None
            if image_path:
            ...
            data = {
                "name": food_name,
                "image": image_filename,  # Add image filename to data
        ...
        # Show image picker before saving
    ...
    def show_food_database(self):
        ...
        popup = Popup(title="Food Database", 
                     content=scroll_view,  # Use ScrollView as content
                     size_hint=(0.9, 0.9),
                     background='',
    ...
    def show_logged_foods(self):
        ...
        popup = Popup(title="Logged Foods", 
                     ...
                     size_hint=(0.9, 0.9),
                     background='',
                     background_color=(1, 0.5, 0.5, 1))  # Light red background color
        popup.open()
    ...
    def show_food_details(self, food):
        ...
        if "serving_size" in food:
            layout.add_widget(Label(text=f"Serving Size: {food['serving_size']}"))
        for key, value in food.items():
            # Skip both serving_size and image when displaying text details
            if key not in ["serving_size", "image"]:
                layout.add_widget(Label(text=f"{key.capitalize()}: {value}"))
...
class DailyIntakeScreen(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        # Add this line to register the custom option class
