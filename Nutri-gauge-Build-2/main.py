...
class FoodLogScreen(BoxLayout):
    ...
    def show_food_database(self):
        try:
            response = requests.get("http://localhost/foods.php")  
            foods = response.json()
        ...
        for food in foods:
            btn = Button(text=f"{food['name']} ({food['calories']} kcal)", size_hint_y=None, height=40)
            btn.bind(on_release=lambda btn, food=food: self.add_food_from_db(food))
